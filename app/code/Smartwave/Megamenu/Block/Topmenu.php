...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function getMegamenuCache() {
        if ($this->moduleManager->isEnabled('Pnp_CustomCacheType')) {
            $cacheKey  = \Pnp\CustomCacheType\Model\Cache\Type\CacheType::TYPE_IDENTIFIER;

            $cache = $this->_cache->load($cacheKey);
            if($cache) {
                $cacheData = $this->serializer->unserialize($cache);
                return $cacheData['block_html'];
            }
        }
        return false;
    }

    public function saveMegamenuCache($html) {
        if ($this->moduleManager->isEnabled('Pnp_CustomCacheType')) {
            $cacheKey  = \Pnp\CustomCacheType\Model\Cache\Type\CacheType::TYPE_IDENTIFIER;
            $cacheTag  = \Pnp\CustomCacheType\Model\Cache\Type\CacheType::CACHE_TAG;
            $data = ['block_html' => $html];
            $this->_cache->save(
    ...
    {
        $cacheHtml = $this->getMegamenuCache();
        if ($cacheHtml) {
            return $cacheHtml;
        }
