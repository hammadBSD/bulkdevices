...
class Frontend extends Template
{
    ...
    public function getPostInfo($post)
    {
        ...
        $html           = __('<div class="col-6">'.
            '<i class="mp-blog-icon mp-blog-calendar-times"></i> %1',
            $this->getDateFormat($post->getPublishDate()).'</div>'
        );

        if ($categoryPost = $this->getPostCategoryHtml($post)) {
            $html .= __('<div class="col-6">'.'Posted in %1'.'</div>', $categoryPost);
        }

        $author = $this->helperData->getAuthorByPost($post);
        if ($author && $author->getName() && $this->helperData->showAuthorInfo()) {
            $aTag = '<div class="col-6">'.'<a class="mp-info" href="' . $author->getUrl() . '">'
                . $this->escapeHtml($author->getName()) . '</a>'.'</div>';
            $html .= __('<div class="col-6">'.'<i class="mp-blog-icon mp-blog-user"></i> %1'.'</div>', $aTag);
        }

        if ($this->getCommentinPost($post)) {
            $html .= __(
                '<div class="col-6">'.'<i class="mp-blog-icon mp-blog-comments" aria-hidden="true"></i> %1'.'</div>',
        ...
        if ($post->getViewTraffic()) {
            $html .= __(
                '<div class="col-6">'.'<i class="mp-blog-icon mp-blog-traffic" aria-hidden="true"></i> %1'.'</div>',
                $post->getViewTraffic()
            );
        }

        if ($couldLike > 0) {
            $html .= __('<div class="col-6">'.'<i class="mp-blog-icon mp-blog-thumbs-up" aria-hidden="true"></i> %1'.'</div>', $couldLike);
        }
