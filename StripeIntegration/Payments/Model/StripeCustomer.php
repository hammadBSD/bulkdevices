...
class StripeCustomer extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function attachPaymentMethod($paymentMethodId)
    {
        ...
        {
            $this->helper->log($e->getMessage(), $e->getTraceAsString());
            return $this->paymentsHelper->dieWithError("Could not load payment method: " . $e->getMessage());
        }
        ...
        {
            $this->helper->logError("Payment method $paymentMethodId belongs to {$paymentMethod->customer} but was used with a subscription by " . $this->getStripeId());
